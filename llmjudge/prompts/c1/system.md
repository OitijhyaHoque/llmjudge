...
## A. Contradicts the dataset's own definitions
1. diabetesMed=No but some drug column is active; or diabetesMed=Yes but every drug column is No.
2. change=No but some drug is Up/Down, or two or more drug columns are active.
3. change=Ch but no drug is Up/Down and fewer than two drug columns are active.
4. num_medications is smaller than the number of active diabetes drug columns.
5. number_diagnoses is smaller than the number of non-missing diag_1..3.
6. The patient died (discharge_disposition_id 11, 19, 20 or 21) but label=1.
7. A value outside its allowed set or range (see Section 2).

## B. Clinically impossible
