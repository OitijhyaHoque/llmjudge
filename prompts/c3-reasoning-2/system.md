...
Return only this JSON object as your answer, on the last line and with nothing after it, and
write "short_reason" FIRST, before "verdict":
{"short_reason": "<at most 25 words>", "verdict": "consistent" | "inconsistent" | "unsure"}

The answer must be that JSON object alone: no code fence, double quotes, and `verdict` exactly one
of `consistent`, `inconsistent`, `unsure` in lower case.
